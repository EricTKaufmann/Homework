...
-- My Code
UPDATE EVCARS
SET TopSpeed = RTRIM(TopSped,'km/h')
...
-- My Code 
SELECT TopSpeed, RTRIM(TopSpeed,'km/h') AS TopSped
FROM EVCARS;
...
Update EVCARS
SET TopSpeed = ROUNF(TopSpeed * 0.621371, 1);
...
ALTER TABLE EVCARS 
RENAME COLUMN TopSpeed to topSpeedMPH;
```
<br>
...
SELECT RTRIM(Range,'km') AS Range
FROM EVCARS;
